![cover_image](./assets/ee6a5bab126ff14a4eb72b79d7299e31.jpg)
...
![](./assets/86151495c384d7933cefaf174ed56580.jpg)
...
![](./assets/40e3e3bdebb4da97d5276245b0fcaf98.png)
...
![](./assets/49a2fa7cc5e1e017998af34ea6976727.png)
...
| --- | --- | --- | --- |
| 属性  | 类型  | 说明  | 示例值 |
| `gen_ai.skill.name` | string | Skill 名称 | add\_to\_cart |
| `gen_ai.skill.id` | string | Skill 实例标识，区分灰度/A/B 实验 | skill\_7d1af98c |
...
![](./assets/fb2c0b6c33290474682545a5c0904ed6.png)
...
![](./assets/44a29111013379369d9e2a1dc4a7b5e0.png)
...
![](./assets/02b1f2e00b5b1c48dcd4bcacea76ea9a.png)

##### 案例2：Token级观测，精准定位答非所问的根因

下面这个案例属于典型的“答非所问”案例，比如用户问的是医疗问题，但大模型回复了 LeetCode 解题答案。

打开如下图异常 Trace 的 Token 分析页面，我们一眼能看到首 Token 是"begin\_of\_sentence"。这个 Token 是一个特殊的 Token，简称BOS，它被用来分割毫无关联的两个语料。也就是说，一旦出现了BOS，那么接下来的回答就跟之前的 prompt 毫无关联，自然而然就答非所问了。所以很明显，在任何情况下，回答里面都不应该出现BOS，那么问题定界到为何会出现这个BOS。对于这个案例，无论在用户的回复里面，或者是引擎日志/网关日志里面，都不会显示“begin\_of\_sentence”，而只会显示成空串，所以没有 Token 分析，定位过程会变得复杂。后续我们进一步挖掘发现产出BOS属于大模型的 badcase，解决方案是调整模型或者等后续模型版本优化更新。

![](./assets/7e7d312203b1332caab3ecadf7496cb6.png)
...
![](./assets/51e353e96ba682799f8645eed870f1c4.png)
...
| Agent 调用 | InvokeAgentInvocation | handler.invoke\_agent() | invoke\_agent ShoppingAssistant |
| Agent 创建 | CreateAgentInvocation | handler.create\_agent() | create\_agent SupportAgent |
| 工具执行 | ExecuteToolInvocation | handler.execute\_tool() | execute\_tool get\_weather |
| 向量嵌入 | EmbeddingInvocation | handler.embedding() | embeddings text-embedding-3-small |
| 文档检索 | RetrieveInvocation | handler.retrieve() | retrieve\_documents |
| 文档重排序 | RerankInvocation | handler.rerank() | rerank\_documents |
| 记忆操作 | MemoryInvocation | handler.memory() | memory\_operation search |
...
在上面这段代码中，开发者没有直接操作任何 OTel API——不需要手动创建 Span、设置 SpanKind、挂载 gen\_ai.agent.name 属性、记录 Duration Metrics——这些全部由 ExtendedTelemetryHandler 在 Context Manager 的 \_\_enter\_\_ 和 \_\_exit\_\_ 中自动完成。若在调用过程中抛出异常，Handler 会自动捕获并在 Span 上设置 error.type 属性和错误状态。详细使用过程可以参考文档：  
...
|     |     |     |     |
| --- | --- | --- | --- |
| LoongSuite GenAI SemConv | LoongSuite Java | LoongSuite Python | LoongSuite Go |
