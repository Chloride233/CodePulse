![cover_image](./assets/63701237b0626eb9d91b64d1f7eb15e9.jpg)
...
![图片](./assets/0153ec947a6f124ffdcb3816c8071b67.gif)
...
![图片](./assets/4f739b8a9eacaa432cae7fd71d89e84b.png)
...
![](./assets/83fce0ffb37a635bad5b8c0d18b9b5d6.png)
...
```
rubric: observation_points:   -回答是否基于项目规范/知识库而非通用知识   -推理过程是否清晰连贯   -是否存在幻觉或编造内容scoring:   process_score:0-100   # 过程分   result_score:0-100   # 结果分   is_false_positive:bool   # 是否虚假成功（结果对但过程错）
```
...
| **功能工具**<br><br>（如性能分析、数据处理） | 过程合规性、工具调用正确性 | 是否按预期步骤调用了正确的工具；工具参数是否正确；输出报告是否完整准确 |
| **问题定位**<br><br>（如故障排查、日志分析） | 推理链路、根因准确性 | 推理过程是否逻辑清晰；是否定位到真实根因；排查步骤是否高效无冗余 |
...
![](./assets/f1405a0fb376da85360d3bcb5fce963c.png)
...
| --- | --- | --- | --- | --- |
| output-pos-01 | 正向  | "分析用例1434534" | `file_exists`<br><br> + `response_contains` + `response_format_check` | 产物文件存在、关键结论完整、包含结构化内容 |
| output-pos-02 | 正向  | "分析用例1434534，输出JSON格式报告" | `file_valid_json`<br><br> + `file_json_has_keys` | 格式合规、必填字段齐全 |
...
![](./assets/9f025b3180d31a9c7eaa4f9ca0ee9584.jpg)
...
![](./assets/8ebabc21e03ac47c068dc3b7ce3d3e4a.jpg)
...
![](./assets/c5b7a2b7a5e7c20ab32cf51022d53e0c.png)
...
| --- | --- | --- |
| **关键决策类**<br><br>（如问题定位、故障诊断） | 0%（N/N 全部通过） | 任何一次失败都可能导致误判（无论是幻觉、步骤遗漏还是逻辑错误），不可容忍 |
| **辅助分析类**<br><br>（如性能分析、报告生成） | ≤ 10%（如 10 次最多 1 次失败） | 偶发偏差可接受，但需持续观察 |
| **创意生成类**<br><br>（如代码建议、文案撰写） | ≤ 40% | 输出多样性本身是特性，关注核心逻辑正确性 |
...
![](./assets/65606c4bd6830971ac5d43bb711ec1b3.png)
...
![](./assets/f53889e481ced5d2b557db15def7d511.png)
...
```
单次 Trial:  trial_score = max(0, 100 + 步骤扣分 + 效率扣分 + 报告扣分)达标判定:    trial_pass  = (trial_score ≥ T)，T = 80用例总分:    case_score  = avg(所有 trial_score)  若全部 trial_pass = true                         = 0                       若任一 trial_pass = false
```
...
```
task_group: test_cputask_group_alias:测试CPU相关指标异常情况下的分析tasks:-task_name:test_cpu_nic_queue_interrupt_bindcore_error_single_core_high_load task_desc:CPU瓶颈-网卡队列中断绑核配置异常-单核高负载、si占用高、RSS队列收包均衡 tperf_test_record_id:'1258797' prompt:分析用例{tperf_test_record_id} base_line_trial:   message_id:'269806ca2dd04e259b042345cf7fc313'   session_id:'66254b2c49794713b02cbecd425ebf09'
```
...
![](./assets/f9c6e2b6fc23d70a02d0b5d903ffaed7.png)
...
![](./assets/63855deb770401351f5e67a34d2c40aa.png)

**单个 Trial 执行流程：  
**

![](./assets/3d634014dba55fffdbbd2b5c7522b1c7.png)
...
![](./assets/9e5ee00cc922eb3fb0e06e3b4787f51d.jpg)
...
![](./assets/bbfd8276de5d178c42fd449ba3e611cb.png)

**测评流水线**

基线确认后进入正式测评环节，采用 **pytest 实现全部测评逻辑，用例管理复用 TCase 平台**：

![](./assets/3040f60c8d061770ea61364dc3e7811d.jpg)
